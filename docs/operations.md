...
`npm run setup:publishable-key` idempotently ensures the real Medusa publishable
key titled `Garmops Storefront — Local Development` exists and is linked to the
existing `Default Sales Channel`. The command prints the token only when a new
key is created; record it in the ignored frontend `.env.local` variable
`MEDUSA_PUBLISHABLE_API_KEY`. It never creates a sales channel or catalog
product.
