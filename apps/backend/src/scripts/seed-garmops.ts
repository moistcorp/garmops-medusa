...
export default async function seedGarmops({ container }: ExecArgs) {
  ...
  if (!salesChannelId) throw new Error("Default Sales Channel is required before catalog bootstrap")
  let created = 0
  for (const item of CATALOG) {
    ...
    if (existing[0]) await productService.updateProducts(existing[0].id, payload as never)
