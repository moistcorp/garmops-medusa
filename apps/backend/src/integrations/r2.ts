...
export async function createSignedUpload(input: { key: string; contentType: string; contentLength: number; fileId: string; sha256?: string; expiresIn?: number }) { const metadata = { "file-id": input.fileId, "expected-size": String(input.contentLength), ...(input.sha256 ? { "expected-sha256": input.sha256 } : {}) }; if (useTestDouble()) { injectTestFailure("r2-upload"); return { key: input.key, url: `test://upload/${encodeURIComponent(input.key)}`, metadata } }; const command = new PutObjectCommand({ Bucket: privateBucket(), Key: input.key, ContentType: input.contentType, ContentLength: input.contentLength, Metadata: metadata }); return { key: input.key, url: await getSignedUrl(client(), command, { expiresIn: input.expiresIn ?? 600 }) } }
export async function verifyObject(input: { key: string; fileId: string; expectedBytes: number; sha256?: string }) { const object = useTestDouble() ? (injectTestFailure("r2-verify"), testHeadObject(input.key)) : await client().send(new HeadObjectCommand({ Bucket: privateBucket(), Key: input.key })); if (object.ContentLength !== input.expectedBytes || object.Metadata?.["file-id"] !== input.fileId || object.Metadata?.["expected-size"] !== String(input.expectedBytes)) throw new MedusaError(MedusaError.Types.FORBIDDEN, "Uploaded object failed ownership or size verification"); if (input.sha256 && object.Metadata?.["expected-sha256"] !== input.sha256) throw new MedusaError(MedusaError.Types.INVALID_DATA, "Uploaded object failed checksum verification"); return object }
export async function createSignedDownload(key: string, expiresIn = 300) { if (useTestDouble()) { injectTestFailure("r2-download"); return `test://download/${encodeURIComponent(key)}` }; return getSignedUrl(client(), new GetObjectCommand({ Bucket: privateBucket(), Key: key }), { expiresIn }) }
...
export async function putPrivateObject(input: { key: string; body: Uint8Array; contentType: string; metadata?: Record<string, string> }) {
  if (useTestDouble()) { injectTestFailure("r2-put"); testPutObject(input); return { key: input.key, bucket: privateBucket(), bytes: input.body.byteLength } }
...
export async function readPrivateObjectForScanner(key: string): Promise<AsyncIterable<Uint8Array>> {
  if (useTestDouble()) { injectTestFailure("r2-read"); return testGetObject(key) }
