...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  try {
    const { cart } = await createCustomerCart(req.scope, customerId, "sample", contact.email)
    ...
    const address = body.shipping.address
    await saveCheckout(req.scope, { cartId: cart.id, customerId, email: contact.email, orderNotes: body.orderNotes, shippingAddress: { ...address, first_name: contact.firstName, last_name: contact.lastName, phone: contact.phone, country_code: "in" }, billingAddress: { ...address, first_name: contact.firstName, last_name: contact.lastName, phone: contact.phone, country_code: "in" }, termsVersion: "2026-07-29", privacyVersion: "2026-07-29", requestId: req.requestId })
    ...
    const raced = (await service.listCheckoutIdempotencies({ key: storageKey }))[0]
    if (raced && raced.customer_id === customerId && raced.request_fingerprint === requestFingerprint) {
      const summary = await summarizeCart(req.scope, raced.cart_id, customerId)
      return res.status(200).json({ order: { checkoutPaymentAttemptId: raced.cart_id, alreadyFinalized: false, orderId: null, orderNumber: null, subtotalPaise: summary.subtotalPaise, taxPaise: summary.gstPaise, totalPaise: summary.grandTotalPaise }, requestId: req.requestId })
    }
