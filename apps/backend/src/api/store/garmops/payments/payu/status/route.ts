...
export async function GET(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    return res.json({ status, orderId: event?.order_id ?? null, transactionId: event?.provider_transaction_id ?? null, paymentState: payment?.status ?? null, requestId: req.requestId })
