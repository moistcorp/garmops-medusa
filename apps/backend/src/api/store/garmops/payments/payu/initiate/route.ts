...
import { assertCheckoutReadyForPayment, withCartLock } from "../../../../../../services/garmops-cart"
...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  try {
    return await withCartLock(req.scope, cartId, async () => {
      ...
      try {
        const attempt = await service.createPaymentAttempts({ provider: "payu", cart_id: cartId, customer_id: actorId, payment_session_id: String(session.id), provider_transaction_id: providerTransactionId, expected_amount_paise: ready.authoritativeTotal, cart_revision_hash: ready.cartRevisionHash, snapshot, status: "active", expires_at: expiresAt, invalidated_at: null, completed_at: null, last_error: null })
        ...
        await req.scope.resolve<any>(Modules.CART).updateCarts(cartId, { metadata: { ...(cart.metadata ?? {}), garmops_payment_attempt: { id: attempt.id, providerTransactionId, paymentSessionId: String(session.id), cartId, customerId: actorId, expectedAmountPaise: ready.authoritativeTotal, revisionHash: ready.cartRevisionHash, cartRevisionHash: ready.cartRevisionHash, status: "active", expiresAt: expiresAt.toISOString() } } })
