...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse) {
  ...
  const { result } = await createGarmopsDesignWorkflow(req.scope).run({ input: { customerId, title: body.title.trim(), productSlug: body.productSlug, configuration: body.configuration, quantity } })
