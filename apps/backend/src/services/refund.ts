...
export async function requestPayuRefund(container: { resolve<T>(key: string): T }, input: { paymentId: string; amountPaise?: number; idempotencyKey: string; actorId: string }) {
  ...
  const request = await service.createRefundRequests({ payment_id: input.paymentId, order_id: orderId, amount_paise: amountPaise, idempotency_key: input.idempotencyKey, requested_by: input.actorId, status: "pending", provider_reference: null, last_error: null, submitted_at: null, completed_at: null })
  let updated: Awaited<ReturnType<IPaymentModuleService["refundPayment"]>>
  try {
    updated = await payments.refundPayment({ payment_id: input.paymentId, amount: amountPaise, created_by: input.actorId, note: "Garmops Founder refund", metadata: { garmops_refund_request_id: request.id } })
  } catch (error) {
...
export async function reconcilePayuRefund(container: { resolve<T>(key: string): T }, refundId: string) {
  ...
  const captured = Number(payment.captured_amount ?? payment.amount)
