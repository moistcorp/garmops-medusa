...
export async function processPayuEvent(req: PayuRequest, source: "callback" | "webhook") {
  ...
  const sessions = await payment.listPaymentSessions({ provider_id: "pp_payu" }, { take: 1000 })
  const session = sessions.find((candidate) => String(candidate.data?.txnid ?? (candidate.data?.fields as Record<string, unknown> | undefined)?.txnid ?? "") === transactionId)
  if (!session) return { status: 404, body: { code: "UNKNOWN_PAYU_TRANSACTION", message: "PayU transaction is not recognized" } }
